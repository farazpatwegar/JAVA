...
/*
 * overriding form of the method CAN'T throw 
 * any NEW or BROADER checked excs
 */
	@Override
	public void init() throws ServletException {
		...
		try {
			...
			//dependent : servlet , dependency - user dao (for DAL)
			//depnt object is creating it's own depcy.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) {
				...
				// valid login --1. Create a Cookie to store validated user dtls
...
//				
				if (user.getRole().equals("voter")) {
					 ...
					 * 2. sends temp  redirect resp - SC 302 ,
					 * Header - 1. Location="candidate_list",
					 * 2. Set-Cookie - cookie name -value
					 * resp body -empty
					 * 3. Clnt browser -chks privacy settings
					 * cookies blocked -- cookies won't be stored--can't remember the clnt
					 * accepted -- cookie age --def value -1 =>
					 *  saves it in cache(cookie storage)
					 * 4. clnt browser - sends NEXT req (redirect)
					 * URL -http://host:port/day2.1/candidate_list 
					 * method - GET --> CandidateList Servlet
					 * + adds the cookie in req header
					 */
