...
public class ProcessFormServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			// save student details under request scope i.e it indicates server pull everything is going to happen in one request
		...
		//	pw.flush();
			...
			// 3.2 forward
			rd.forward(request, response);//IllegalStateExc -- can't forward after committing the resp
			/*
			 * WC -- 1. clears the resp buffer
			 * 2. suspends exec of this page n calls 
			 * -next page's doPost --after it rets , 
			 * control comes back to the 1st page BUT it CAN NOT 
			 * generate the dyn resp.
			 * 
			 */
			System.out.println("control came back.....");
			pw.print("<h5>after forward</h5>");
		}
	}
